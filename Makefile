...
# CLANG_VERSION needs llvm-config be compiled (see build/llvm.BUILT)
CLANG_VERSION=$(subst svn,,shell $(ROOT_DIR)/build/llvm/bin/llvm-config --version)
...
build/llvm.BUILT:
	...
	cd build/llvm; cmake -G "Unix Makefiles" \
		...
		$(LLVM_PROJ_DIR)/llvm
	cd build/llvm; $(MAKE) -j 8 llvm-config
